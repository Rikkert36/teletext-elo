...
import { Client, Player, FileParameter } from '../clients/server.generated';
...
const addRankToPlayers = (players: Player[]) : RankedPlayer[] => {
  const sortedPlayers : Player[] = [...players].sort((a, b) => b.rating! - a.rating!);
...
const numberOfPlayersWithHigherRating = (sortedPlayers: Player[], index: number) : number => {
  var player = sortedPlayers[index];
  var counter = index;
  while(counter > -1 && player.rating == sortedPlayers[counter].rating) {
    counter--;
  }
  return counter + 1;
}; 
    ...
    if (players) {
      return players!.map((player, index) => (
        <TableRow key={player.player.id} className={classes.tableRow}>
          ...
          <TableCell className={classes.tableCell + ' ' + classes.otherRowValue}>{player.player.rating}</TableCell>
        </TableRow>
      ));
    }
  };
