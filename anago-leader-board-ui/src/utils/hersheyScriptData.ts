/*
 * Hershey "script simplex" glyph data, vendored.
 *
 * ---------------------------------------------------------------------------
 ...
 * ---------------------------------------------------------------------------
 *
 * That acknowledgement is not decoration: the Hershey licence permits use for any
 * purpose, commercial or otherwise, **on the condition that it travels with the font
 * data**. It stays in this file, which is the file the data is in.
 *
 * **Generated from `scripts.jhf`, not typed.** Ninety-six lines of packed coordinate
 * data is exactly where a transcription slip hides — a single wrong character is one
 * malformed letter in somebody's name, and nothing would ever surface it. Regenerate
 * rather than edit by hand.
 *
 * **Simplex, not complex, and that is the load-bearing choice.** `scriptc.jhf` draws
 * every stem as two parallel strokes to fake weight. Traced with `stroke-dashoffset`
 * that reads as the pen going up one side of the letter and back down the other —
 * the same silhouette-tracing failure that rules out converting an ordinary outline
 * font. Simplex glyphs are true centrelines: one stroke is one movement of a pen, so
 * drawing them in order *is* handwriting. See `hersheyScript.ts`.
 *
 * One line per glyph, ASCII 32..126 in order, so `JHF[code - 32]` is the glyph.
 * Columns: 0-4 glyph number, 5-7 vertex count, 8 left bearing, 9 right bearing,
 * 10+ coordinate pairs offset from 'R', with " R" meaning pen-up.
 */
export const JHF_FIRST_CHAR = 32;

export const JHF: readonly string[] = [
  "  699  1JZ",
